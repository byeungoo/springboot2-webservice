...
import com.hoon.springboot.config.auth.dto.OAuthAttributes;
import com.hoon.springboot.config.auth.dto.SessionUser;
import com.hoon.springboot.domain.user.User;
import com.hoon.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        httpSession.setAttribute("user", new SessionUser(user));    //세션에 user 등록

        return new DefaultOAuth2User(
                Collections.singleton(new SimpleGrantedAuthority(user.getRoleKey())),   //유저 ROLE 세팅
